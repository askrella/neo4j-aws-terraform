...
resource "aws_route53_record" "core_record" {
  ...
  name    = "${var.project}-${var.name}-core${format("%02d", count.index + 1)}.${data.aws_route53_zone.domain[0].name}"
  type    = "A"
  ttl     = "60"
...
resource "aws_route53_record" "cluster_record" {
  count   = "${var.r53_domain == "" ? 0 : 1}"
  zone_id = "${data.aws_route53_zone.domain[0].zone_id}"
  name    = "${var.project}-${var.name}.${data.aws_route53_zone.domain[0].name}"
  type    = "A"
  ttl     = "60"
