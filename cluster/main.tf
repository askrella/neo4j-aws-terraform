...
module "core" {
  source                 = "github.com/skyscrapers/terraform-instances//instance?ref=3.2.0"
  project                = "${var.project}"
  ...
  sgs                    = concat([aws_security_group.sg.id], var.security_group_ids)
...
resource "aws_ebs_volume" "core" {
  ...
  tags = merge(var.tags,
    tomap({"Name": "${var.project}-${var.environment}-${var.name}-coredata${count.index + 1}",
      "Environment": var.environment,
      "Project": var.project}))
}
